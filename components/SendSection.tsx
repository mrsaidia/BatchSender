...
import { BaseError, useAccount, useChainId, useReadContract, useStorageAt, useWaitForTransactionReceipt, useWriteContract } from 'wagmi';
...
const SendSection = () => {
    ...
    //   let symboltoken = await readContract(config, {
    //     abi: ERC20,
  ...
  async function sendTokenDifferentAmount() {
    if (!address) {
      toast.error('Please connect your wallet!', {
        autoClose: 3000
      });
    ...
    writeContractAsync({ 
      address: `0x${tokenAddress.trim().substring(2)}`, 
      abi: ERC20,
      functionName: 'approve', 
      args: [DISPERSE_ADDRESS[chainId], 99999999999999999999999999999], 
      chainId: chainId,
    }) 
    writeContractAsync({ 
      address: `0x${DISPERSE_ADDRESS[chainId].substring(2)}`, 
      ...
      args: [tokenAddress, addresses.split('\n'), amountListFinal], 
      chainId: chainId,
    }) 
  }
  catch (error) {
      ...
      <>
          ...
          >
           ...
           <div className={styles.container}>
            ...
            <br></br>
            <button onClick={sendTokenDifferentAmount} className={styles['button-3']}>Send</button>
            </>}
